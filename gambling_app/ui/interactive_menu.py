...
class InteractiveMenu:
    ...
    def display_main_menu(self):
        ...
        console.print("1. Create Profile  | 2. View Status      | 3. Edit Preferences")
        console.print("4. Reset Limits")
        console.print("[dim]-- Banking --[/dim]")
        console.print("5. Deposit Funds   | 6. Stake History")
        console.print("[dim]-- Game Session --[/dim]")
        console.print("7. Start Session   | 8. Pause/Resume     | 9. End Session")
        console.print("10. View Past Session Summary")
        console.print("[dim]-- Betting Room --[/dim]")
        console.print("11. Manual Bet     | 12. Auto-Play (Strategy)")
        console.print("[dim]----------------------------[/dim]")
        console.print("13. Exit")
        return Prompt.ask("Select an option", choices=[str(i) for i in range(1, 14)])
        ...
        while True:
            ...
            try:
                if choice == "1":
                    ...
                    min_bet = self.input_handler.ask_float("Minimum Bet Limit (₹)", "min_bet")
                    max_bet = self.input_handler.ask_float("Maximum Bet Limit (₹)", "max_bet")

                    # UC6 Validation before proceeding
                    try:
                        self.input_handler.validator.validate_initial_stake(Decimal(str(stake)), Decimal('10'), Decimal('1000000'))
                        self.input_handler.validator.validate_limits(Decimal(str(l_thresh)), Decimal(str(w_thresh)), Decimal(str(stake)))
                    except ValidationException as ve:
                        console.print(f"\n[bold red]✖ Configuration Error:[/bold red] {ve}")
                        self.input_handler.repo.log_event(ve.error_type.value, "ERROR", str(ve), ve.field, ve.attempted_value)
                        continue 

                    res = self.gambler_ctrl.register_new_gambler(
                        username, email, stake, w_thresh, l_thresh, min_bet, max_bet
                    )
                    ...
                    gid = int(self.input_handler.ask_float("Enter Gambler ID", "gambler_id"))
                    min_bet = self.input_handler.ask_float("New Min Bet (₹)", "min_bet")
                    max_bet = self.input_handler.ask_float("New Max Bet (₹)", "max_bet")
                    
                    with console.status("Updating..."):
                        res = self.gambler_ctrl.update_preferences(gid, min_bet, max_bet)
